...
void CreateDate(int &day, int &month, int &year) { // Проверяем дату, на вход ссылки на три целочисленные(ссылки, чтобы целочисленные переменные, которые задаются при вызове функции, сохраняли именения, которые произойдут с ними в функции)
	bool Trust = true;
	while (Trust)
	{
